<script lang="ts">
    ...
    onMount(() => {
        const handoff = scrapeHandoff.consume();
        if (handoff) {
            targetUrl = handoff.url;
            usernameSelector = handoff.username_selector || "";
            passwordSelector = handoff.password_selector || "";
            submitSelector = handoff.submit_selector || "";
            successSelector = handoff.success_selector ?? "";
            failureSelector = handoff.failure_selector ?? "";
            captchaSelector = handoff.captcha_selector ?? "";
            consentSelector = handoff.consent_selector ?? "";
            totpSelector = handoff.totp_selector ?? "";
            showFormConfig = true;

            // Also apply domain profile settings if we have a URL
            if (handoff.url) {
                try {
                    const dp = resolveDomainProfile(handoff.url);
                    patchTls = dp.patch_tls;
                    launchDelay = dp.min_attempt_gap_ms;
                } catch {}
            }

            if (handoff.detection_note) {
                scrapeSuccess = `✓ Imported from URL test: ${handoff.detection_note}`;
            } else {
                scrapeSuccess = "✓ Form selectors imported from URL test";
            }
        }
    });
    ...
    async function launchSession() {
        if (!sessionProfileId) {
            sessionError = "Pick a profile first.";
            return;
        }
        sessionError = "";
        ...
        try {
            ...
            const port = await profileStore.launchProfile(
                sessionProfileId,
    ...
    let resultStatusFilter = $state<
        "all" | "success" | "failed" | "blocked" | "error"
    >("all");
    let resultOutcomeFilter = $state<
        | "all"
        | "success"
        | "wrong_credentials"
        | "captcha_block"
    ...
    let domainProfile = $derived(
        targetUrl
            ? (() => {
                  try {
                      return resolveDomainProfile(targetUrl);
                  } catch {
                      return null;
                  }
              })()
    ...
    let threatColor = $derived(
        domainProfile?.threat_level === "paranoid"
            ? "var(--error)"
            : domainProfile?.threat_level === "high"
              ? "var(--warning)"
              : domainProfile?.threat_level === "medium"
                ? "var(--accent)"
                : "var(--success)",
    );
        ...
        try {
            ...
            if (!config) {
                ...
                if (scraped.confidence > 0) {
                    ...
                    if (scraped.isSPA) {
                        features.push(
                            `SPA (${scraped.spaFramework || "unknown"})`,
                        );
                    }
                    if (scraped.hasCaptcha) {
                        features.push(
                            `CAPTCHA: ${scraped.captchaProviders?.join(", ") || "detected"}`,
                        );
                    }
                    if (scraped.hasMFA) {
    ...
    function filterResults(
        ...
        if (resultStatusFilter !== "all") {
            filtered = filtered.filter((item) => {
                switch (resultStatusFilter) {
                    case "success":
                        return item.cred.status === "success";
                    case "failed":
                        return item.cred.status === "failed";
                    case "blocked":
                        return (
                            item.cred.status === "soft_blocked" ||
                            item.cred.status === "hard_blocked"
                        );
                    case "error":
                        return item.cred.status === "error";
                    default:
                        return true;
                }
            });
        ...
        if (resultOutcomeFilter !== "all" && filtered.length > 0) {
            filtered = filtered.filter(
                (item) => item.result?.outcome === resultOutcomeFilter,
            );
        }
        ...
        filtered.sort((a, b) => {
            ...
            switch (sortBy) {
                ...
                case "duration":
                    cmp =
                        (a.result?.duration_ms ?? 0) -
                        (b.result?.duration_ms ?? 0);
                    break;
                default: // index
                    cmp = a.index - b.index;
            }
...
<div class="page">
    ...
    <div class="page-header">
        <div class="header-left">
            ...
            {#if activeRun}
                <span
                    class="run-badge"
                    class:run-badge-live={activeRun.status === "running"}
                >
        ...
        <div class="header-actions">
            ...
            {#if activeRun && (activeRun.status === "completed" || activeRun.status === "aborted")}
                <button
                    class="btn btn-secondary btn-sm"
                    onclick={() =>
                        automationStore.downloadRunReport(activeRun!.id)}
                >
                    ↓ Report
                </button>
                <button
                    class="btn btn-secondary btn-sm"
                    onclick={() =>
                        automationStore.downloadSuccessCreds(activeRun!.id)}
                >
                    ↓ Hits
                </button>
                {#if activeRun.results.some((r) => r.session_exported)}
                    <button
                        class="btn btn-secondary btn-sm"
                        onclick={() =>
                            automationStore.downloadAllSessions(activeRun!.id)}
                    >
                        ↓ Sessions
                    </button>
                {/if}
    ...
    <div class="session-bar">
        ...
        {#if sessionProfile}
            <span
                class="session-fp-badge"
                title="UA: {sessionProfile.fingerprint.user_agent}"
            >
                <svg
                    width="9"
                    height="9"
                ...
                · Chrome {sessionProfile.fingerprint.ua_brands.find(
                    (b) => b.brand === "Google Chrome",
                )?.version ?? "?"}
                {#if sessionProfile.fingerprint.webgl_renderer}
                    · {sessionProfile.fingerprint.webgl_renderer
                        .replace(/ANGLE \([^,]+, /i, "")
                        .replace(/ Direct3D.*/, "")}
                {/if}
            </span>
        ...
        {#if bridgeConnected}
            <span
                class="session-live-dot"
                title="Session live on ws://localhost:{bridgeStore.port}"
                >●</span
            >
            <span class="session-live-label"
                >{(() => {
                    try {
                        return (
                            new URL(bridgeStore.currentUrl ?? "").hostname ||
                            "connected"
                        );
                    } catch {
                        return "connected";
            ...
            >
            <button class="btn btn-danger btn-xs" onclick={stopSession}
                >Stop</button
            >
        {:else}
            <button
                    ...
                    <div class="setup-card">
                        ...
                        {#if domainProfile}
                            <div class="domain-badge">
                                <span class="domain-host mono"
                                    >{domainProfile.hostname}</span
                                >
                                <span
                                    class="domain-threat"
                    ...
                    <div class="setup-card">
                        ...
                        {:else}
                            ...
                            >
                                ...
                                <label
                                    class="btn btn-secondary btn-sm upload-label"
                                >
                                    Browse
                                    <input
                                        type="file"
                            ...
                            <div class="cred-list">
                                <div class="cred-list-header">
                                    <span class="section-label"
                                        >{drafts.length} credential{drafts.length !==
                                        1
                                            ? "s"
                                            : ""} queued</span
                                    >
                                ...
                                <div class="cred-rows">
                                    {#each drafts.slice(0, 50) as cred (cred.id)}
                                        <div class="cred-row">
                                            <span
                                                class="cred-user mono truncate"
                                                >{cred.username}</span
                                            >
                                            <span class="cred-pass mono"
                                                >{"•".repeat(
                                                    Math.min(
                                    ...
                                    {/each}
                                    {#if drafts.length > 50}
                                        <div class="cred-overflow">
                                            +{drafts.length - 50} more
                                        </div>
                                    {/if}
                                </div>
                            </div>
                        ...
                        <div class="card-title">
                            ...
                            {#if selectedProfiles.length > 0}
                                <span class="card-count"
                                    >{selectedProfiles.length}</span
                                >
                            {/if}
                        </div>

                        <p class="card-hint">
                            Select profiles to draw from. Each credential
                            attempt uses one profile. The runner rotates through
                            the pool on soft signals.
                        </p>
                            ...
                            {:else}
                                {#each profiles as profile (profile.id)}
                                    ...
                                    >
                                        ...
                                        <span
                                            class="dot"
                                            class:dot-running={profile.status ===
                                                "running"}
                                            class:dot-error={profile.status ===
                                                "error"}
                                            class:dot-idle={profile.status ===
                                                "idle"}
                                        ></span>
                ...
                <div class="setup-right">
                    ...
                    <div class="setup-card">
                        ...
                        >
                            ...
                            <span
                                ...
                                class:open={showFormConfig}>›</span
                            >
                            {#if targetUrl}
                                <button
                        ...
                        {#if showFormConfig}
                            <div class="form-fields fade-in">
                                    ...
                                    <div class="message message-error">
                                        <div class="error-title">
                                            ...
                                            >
                                                ...
                                                />
                                                <path
                                                    d="M7 4v3M7 10a0.5 0.5 0 1 1 0 1 0.5 0.5 0 0 1 0 -1"
                                                    stroke="currentColor"
                                                    stroke-width="1.2"
                                                    stroke-linecap="round"
                                                />
                                        ...
                                        <div class="error-help">
                                            ...
                                            <ul>
                                                ...
                                                <li>
                                                    See <a
                                                        href="FORM_SCRAPER_TROUBLESHOOTING.md"
                                                        target="_blank"
                                ...
                                <label class="toggle-row">
                                    <input
                                        type="checkbox"
                                        bind:checked={exportSession}
                                    />
                                    <span
                                        >Export session state on success
                                        (cookies + localStorage + IDB)</span
                                    >
                                </label>
                            </div>
                        {/if}
                    ...
                    <div class="setup-card">
                        <div class="card-title">
                            ...
                            >
                                <path
                                    d="M6 1.5L1.5 4v4l4.5 2.5 4.5-2.5V4L6 1.5z"
                                    stroke="currentColor"
                                    stroke-width="1.3"
                                    stroke-linejoin="round"
                                />
                        ...
                        <div class="options-grid">
                            <label class="toggle-row">
                                <input
                                    type="checkbox"
                                    bind:checked={domainAwareNoise}
                                />
                                <span>Domain-aware canvas/audio/font noise</span
                                >
                                {#if domainProfile}
                                    <span class="tag tag-accent"
                                        >{domainProfile.threat_level}</span
                                    >
                                {/if}
                            </label>
                            ...
                            <label class="toggle-row">
                                <input
                                    type="checkbox"
                                    bind:checked={rotateEvery}
                                />
                                <span
                                    >Rotate proxy + fingerprint every attempt</span
                                >
                            </label>
                        </div>
                        ...
                        <div class="field-grid-2 mt">
                            <div class="field-group">
                                ...
                                <span class="field-hint"
                                    >Rotate after N soft signals</span
                                >
                            </div>
                            <div class="field-group">
                                ...
                                />
                                <span class="field-hint"
                                    >For error / soft-blocked creds</span
                                >
                            </div>
                            <div class="field-group">
                    ...
                    <div class="launch-block">
                        ...
                        {/if}
                        <button
                            class="btn btn-primary btn-lg launch-btn"
                            onclick={handleStart}
                            disabled={!canStart}
                        >
                            <svg
                            ...
                            >
                                <path
                                    d="M2.5 1.5l9 5-9 5V1.5z"
                                    fill="currentColor"
                                />
                            </svg>
                            Launch {drafts.length > 0
                                ? `(${drafts.length} credential${drafts.length !== 1 ? "s" : ""})`
                                : ""}
                        </button>
                    </div>
                </div>
            ...
            <div class="run-layout">
                {#if !activeRun}
                    <div class="empty" style="flex:1">
                        ...
                        <span class="empty-hint"
                            >Configure and launch a run from the Setup tab.</span
                        >
                        <button
                            class="btn btn-primary btn-sm"
                            onclick={() => {
                ...
                {:else}
                    ...
                    <div class="progress-bar-wrap">
                        <div class="progress-bar-track">
                            <div
                                ...
                                style:width="{(run.stats.success /
                                    run.stats.total) *
                                    100}%"
                            ></div>
                            <div
                                class="progress-bar-fill failed"
                                style:width="{(run.stats.failed /
                                    run.stats.total) *
                                    100}%"
                            ></div>
                            <div
                                class="progress-bar-fill blocked"
                                style:width="{((run.stats.soft_blocked +
                                    run.stats.hard_blocked) /
                                    run.stats.total) *
                                    100}%"
                            ></div>
                            <div
                                class="progress-bar-fill error"
                                style:width="{(run.stats.error /
                                    run.stats.total) *
                                    100}%"
                            ></div>
                        </div>
                        <span class="progress-pct mono"
                            >{progressPct(run.stats)}%</span
                        >
                    </div>

                    <!-- Target info -->
                    <div class="run-meta">
                        <span class="run-target mono truncate"
                            >{run.config.form.url}</span
                        >
                        {#if run.config.domain_profile}
                            <span class="tag" style:color={threatColor}
                                >{run.config.domain_profile.threat_level}</span
                            >
                        {/if}
                        ...
                        {/if}
                        <span class="run-time mono"
                            >{relTime(run.started_at)}</span
                        >
                    </div>
                    ...
                    <div class="results-filter-bar">
                        ...
                        <div class="filter-buttons">
                            ...
                            <select
                                class="filter-select"
                                bind:value={resultOutcomeFilter}
                            >
                                <option value="all">All Outcomes</option>
                                <option value="success">Success</option>
                                <option value="wrong_credentials"
                                    >Wrong Creds</option
                                >
                        ...
                        <span class="filter-count"
                            >{filterResults(run.credentials, run.results, run)
                                .length} / {run.credentials.length}</span
                        >
                    </div>
                        ...
                        <table class="results-table">
                            ...
                            <tbody>
                                {#each filterResults(run.credentials, run.results, run) as item (item.cred.id)}
                                    ...
                                    <tr
                                        class:row-success={cred.status ===
                                            "success"}
                                        class:row-running={cred.status ===
                                            "running"}
                                        class:row-error={cred.status ===
                                            "failed" ||
                                            cred.status === "hard_blocked"}
                                    >
                                        <td class="mono muted"
                                            >{item.index + 1}</td
                                        >
                                        ...
                                        <td>
                                            <span
                                                class="mono muted"
                                                title={cred.profile_id ?? ""}
                                            >
                                                {cred.profile_id
                                                    ? cred.profile_id.slice(
                                                          0,
                                                          8,
                                                      )
                                                    : "—"}
                                            ...
                                            <div class="row-action-btns">
                                                ...
                                                {#if result?.session_exported && result.session_state}
                                                    <button
                                                        ...
                                                        onclick={() =>
                                                            automationStore.downloadSessionState(
                                                                result!
                                                                    .session_state!,
                                                            )}
                                                    >
                                                ...
                                                {#if cred.status === "pending"}
                                                    <button
                                                        ...
                                                        title="Skip"
                                                        onclick={() =>
                                                            automationStore.skipCredential(
                                                                run.id,
                                                                cred.id,
                                                            )}
                                                    >
                                                                ...
                                                                >
                                                                    {#each result.rotation_events as evt}
                                                                        ...
                                                                        >
                                                                            ...
                                                                            <span
                                                                                class="mono muted"
                                                                                >{evt.old_profile_id.slice(
                                                                                    0,
                                                                                    8,
                                                                                )}
                                                                                →
                                                                                {evt.new_profile_id.slice(
                                                                                    0,
                                                                                    8,
                                                                                )}</span
                                                                            >
                    ...
                    {#if run.rotation_log.length > 0}
                        <div class="rotation-panel">
                            <div class="panel-title">
                                Rotation Log — {run.rotation_log.length} event{run
                                    .rotation_log.length !== 1
                                    ? "s"
                                    : ""}
                            </div>
                            <div class="rot-log-list">
                                {#each run.rotation_log
                                    .slice()
                                    .reverse() as evt}
                                    <div class="rot-log-entry">
                                        ...
                                        <span
                                            ...
                                            >{evt.old_profile_id.slice(0, 8)} → {evt.new_profile_id.slice(
                                                0,
                                                8,
                                            )}</span
                                        >
            ...
            <div class="history-layout">
                {#if runs.length === 0}
                    <div class="empty" style="flex:1">
                        ...
                        <span class="empty-hint"
                            >Completed runs appear here.</span
                        >
                    </div>
                {:else}
                        ...
                        >
                            <div class="history-card-header">
                                <span
                                    ...
                                    >{truncate(run.config.form.url, 40)}</span
                                >
                                <div class="history-status">
                                    <span
                                        class="pill {run.status === 'completed'
                                            ? 'pill-success'
                                            : run.status === 'running'
                                              ? 'pill-accent'
                                              : run.status === 'paused'
                                    ...
                                    >
                                        ...
                                        >
                                            <path
                                                d="M1.5 2.5h6M3.5 2.5V1.5h2v1M2 2.5l.5 5h4l.5-5"
                                                stroke="currentColor"
                                                stroke-width="1.1"
                                                stroke-linecap="round"
                                                stroke-linejoin="round"
                            ...
                            <div class="history-stats">
                                <span class="stat-chip success"
                                    ><span class="stat-n"
                                        >{run.stats.success}</span
                                    ><span class="stat-l">hit</span></span
                                >
                                <span class="stat-chip error"
                                    ><span class="stat-n"
                                        >{run.stats.failed}</span
                                    ><span class="stat-l">fail</span></span
                                >
                                <span class="stat-chip warn"
                                    ><span class="stat-n"
                                        >{run.stats.soft_blocked}</span
                                    ><span class="stat-l">block</span></span
                                >
                                <span class="stat-chip muted"
                                    ><span class="stat-n"
                                        >{run.stats.total}</span
                                    ><span class="stat-l">total</span></span
                                >
                            </div>
